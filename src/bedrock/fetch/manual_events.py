...
To hendelses-baserte datakilder:
...
Sub-fase 12.5+ session 113: BDI-fetcheren flyttet til
``bedrock.fetch.shipping`` som del av Baltic-suite-utvidelsen
(BDI+BCI+BPI+BSI long-format).

CLI-kommandoer for append/list er i ``bedrock.cli.manual_events``.

Bruk:
    from bedrock.fetch.manual_events import (
        fetch_export_events, fetch_disease_alerts,
    )
...
__all__ = ["fetch_disease_alerts", "fetch_export_events", "fetch_igc"]
