"""A8 — NOPA Crush Report PDF smoke-test.

Sub-fase 12.7 D0 (session 126). Verifiserer at NOPA (National Oilseed
Processors Association) månedlig crush-rapport er tilgjengelig som PDF.

Endepunkt: https://www.nopa.org/news-resources/monthly-crush-report/

PDF-er er typisk navngitt med dato i filnavn. Smoke-strategi:
1. Last hovedsiden og finn PDF-lenker
2. Forsøk å laste én PDF og verifiser at pypdf/pdftotext kan parse
"""
...
URL_INDEX = "https://www.nopa.org/resources/nopa-monthly-crush-report/"
...
def main() -> None:
    ...
    if not pdf_links:
        print("  RISK: ingen direkte PDF-lenker, JS-rendering trolig")
        return

    # Forsøk å laste første PDF
    first_pdf = pdf_links[0]
    print(f"\nForsøk å laste PDF: {first_pdf}")
    time.sleep(1.5)
    start = time.monotonic()
    code2, data2 = fetch(first_pdf)
    elapsed = time.monotonic() - start
    print(
    ...
    if code2 == 200 and isinstance(data2, bytes) and data2[:4] == b"%PDF":
        ...
        print("  PDF-magic ikke detektert eller error")
