...
def test_strong_bull_over_10pct_returns_one(store: DataStore) -> None:
    ...
    score = currency_cross_trend(store, "Sugar", {"source": "BRLUSD", "lookback": 30})
    assert score == 1.0
...
def test_default_lookback_is_thirty(store: DataStore) -> None:
    _add_closes(store, "BRLUSD", "D1", _pct_step(0.20, 0.15))
    score_default = currency_cross_trend(store, "Sugar", {"source": "BRLUSD"})
    score_explicit = currency_cross_trend(store, "Sugar", {"source": "BRLUSD", "lookback": 30})
...
def test_custom_tf_is_used(store: DataStore) -> None:
    ...
    score_default = currency_cross_trend(store, "Sugar", {"source": "BRLUSD", "lookback": 30})
...
def test_instrument_argument_is_ignored(store: DataStore) -> None:
    ...
    score = currency_cross_trend(store, "Coffee", {"source": "BRLUSD", "lookback": 30})
