...
- **Sub-fase 12.5 ÅPEN 2026-04-25** — debt-rydding før observasjonsvinduet kan gi mening. Roadmap: Block A drivere (~3-4 sessioner), Block B agri-drivere (~4-5), Block C instrumenter (~3-5), Block D polish (~2-3). Totalt 12-17 sessioner.
  ...
  - **109-115:** én fetcher per session per § 7.5: seismic (109), cot_euronext (110), conab (111), unica (112), shipping (113 konsolidering med bdi), news_intel (114 UI-only), crypto_sentiment (115 UI-only).
...
- **Blocked:** nei.
- **Aktive systemd-timere:** 6 system-installerte (calendar_ff [session 105], cot_ice [session 106], signals-all, monitor, compare, server [service]) + 9 user-installerte (prices, cot_disaggregated/legacy, fundamentals, weather, crop_progress, bdi, eia_inventories [session 107], **comex [session 108, ny]**). Andre fetch-timer-units (enso, wasde) er generert i `/home/pc/bedrock/systemd/` men IKKE aktivert.
...
- **Drivere:** 26 registrert (25 + `comex_stress` ny session 108).
- **Bedrock-fetchere:** 13 totalt (prices, cot_disaggregated, cot_legacy, fundamentals, weather, enso, wasde, crop_progress, bdi, calendar_ff [session 105], cot_ice [session 106], eia_inventories [session 107], **comex [session 108]**). 7 gjenstår å port per § 7.5 (sessions 109-115).
...
- **Sub-fase 12.5+ scope:** 11 ikke-portede cot-explorer-fetchere (PLAN § 7.5) + ADR-007 strategi + ADR-008 mapping. Sessions 105-108 lukket (4/11). Sessions 109-115 (7/11) + 116-117 (Phase D) gjenstår.
...
- **Next task:** **Session 109** — `fetch_seismic.py` → `bedrock/fetch/seismic.py` med `mining_disruption`-driver for Gold/Silver/Copper/Platinum. USGS GeoJSON M≥4.5 i mining-regioner. PLAN § 7.5 rad 5.
