...
- **Sub-fase 12.10 LUKKET 2026-05-02** (tag `v0.12.10-fase-12.10-LUKKET`) — driver-rebalansering. 9 bunker tagget `v0.12.10-bunke1` til `v0.12.10-bunke9`. Levert: ~50 nye drivere registrert + 2 driver-default-bumps; ~17 drivere/endringer DEFERRED for follow-up. Backfill: ~60 000+ nye rader på 30+ datakilder (FRED/Yahoo/NOAA/ICE/EIA/USDM-states). 100+ nye tester, pyright 0 errors, 0 grade-flips i finale (drivere ikke YAML-wired ennå). Bot fortsatt på cTrader demo hele veien.
  - **Bunke 1 (3 bug-fixer):** COT released_at look-ahead-fix (release_calendar-modul + AsOfDateStore filter på 5 store-getters + schema-utvidelse på 6 tabeller, 23 733 rader migrert), min_samples-guards på 4 sparsom-data-drivere (unica_change/disease_pressure/export_event_active/comex_stress), event_distance-audit (regresjons-vakter mot `actual`-kolonne).
  - **Bunke 2 (#4 + #6):** 4 cot_ice_mm_pct-wirings i Cocoa/Coffee/Sugar/Wheat cross-familier (ICE_MARKETS-fetcher utvidet med softs, 2420 rader backfilt). news_intel_severity_veto-driver registrert. **#5 *_surprise DEFERRED** (FF mangler actual; FRED-cross-source krever ny data-arkitektur).
  - **Bunke 3 (#7-#14):** 14 nye FRED-drivere i `macro_bunke3.py` — t10y3m, t_bill_3mo_yield, hy_oas_change, initial_claims_z, continuing_claims_z, industrial_production_yoy, cfnai_3mma, umich_sentiment_z, jolts_openings_yoy, anfci_z, m2_yoy, vix9d_vix_ratio, dollar_index_breadth, fomc_decision_distance. Backfill: 26 537 rader (19 FRED-serier + 8 DEX-pairs). **ism_pmi_level DEFERRED** (FRED NAPMPMI 404).
  - **Bunke 4 (#15-#18):** 8 drivere i `macro_bunke4.py` — move_index_z, vvix_z, gvz_z, ovx_z, cboe_skew_z, noaa_oni_index, noaa_pdo_index, intraday_atr_h1. Backfill: 17 330 rader (Yahoo vol + NOAA ONI/PDO). **3 DEFERRED:** cboe_pcr_total_extreme, cboe_pcr_equity_only, cboe_vix_term_curve, noaa_enso_forecast_3mo (alle krever annen kilde enn Yahoo).
  - **Bunke 5 (#19 partial):** USDM state-statistics-fix i fetcher (FIPS-konvertering for IA/TX/CA/KS/ND); 2700 rader backfilt for 5 stater. `drought_monitor`-driver støtter allerede `aoi`-param — wirings utsatt. **#20 NASS yield/grain_stocks DEFERRED** (krever NASS QuickStats-utvidelse). **#21 FAS china/eu DEFERRED** (krever YAML-wirings + tester).
  - **Bunke 6 (#22):** 6 thin-wrapper EIA-drivere i `macro_bunke6.py` — eia_distillate_change, eia_propane_change, eia_refinery_utilization_z, eia_petroleum_supplied, eia_imports_crude, eia_gasoline_demand. Backfill: 11 180 rader (6 EIA-serier 1982-2026). **eia_natgas_processing DEFERRED** (krever monthly natgas-route).
  - **Bunke 7 (#23 + #26 partial):** 7 drivere i `macro_bunke7.py` — agsi_germany_pct, agsi_netherlands_pct, agsi_italy_pct (thin wrappers), agsi_withdrawal_rate, agsi_injection_rate (z-score på new columns), cot_oi_change, cot_commercial_extreme. **DEFERRED:** ALSI (#24, ny GIE-route), IIP REMIT (#25, ny IIP-route), cot_concentration_top4 (Conc_Net ikke i schema), cot_swap_dealer_skew (Swap kun i TFF).
  - **Bunke 8 (#28):** 2 USGS seismic-drivere i `macro_bunke8.py` — seismic_m6_global_24h, seismic_chile_peru_copper. **#27 Treasury auctions DEFERRED** (ingen data). **#29 crypto_sentiment_extreme DEFERRED** per spec (vent til 100+ rader, ~juli 2026).
  - **Bunke 9 (2 av 13):** Driver-default-bumps levert: #33 mining_disruption min_magnitude 4.5→5.5, #35 comex_stress min_samples 20→180. **DEFERRED 11:** YAML-rebalanseringer (#30, #31, #32, #34, #42 — per § 22.1 live-demo-validering først), driver-impl-rewrites (#36-#41 — substantial refactors).
- **Sub-fase 12.10 follow-up Spor A1 LUKKET 2026-05-02** (tag `v0.12.10-followup-a1`). Første runde YAML-wirings av drivere fra 12.10-bunkene; per § 22.1 PRIMÆR-spor (live-demo-validering først, så utvide). 8 driver-wirings i 6 instrumenter:
  - SP500 + Nasdaq risk: `cboe_skew_z@0.10` (alle hor) + `vix9d_vix_ratio@0.10` (SCALP/SWING). vol_regime trimmet 0.59→0.39.
  - CrudeOil + Brent macro: `ovx_z@0.15`. real_yield 0.15→0.10, dxy 0.35→0.25.
  - Gold macro: `gvz_z@0.10`. real_yield 0.35→0.30, etf_holdings_change 0.15→0.10.
  - Copper macro: `seismic_chile_peru_copper@0.10`. real_yield 0.25→0.20, mining_disruption 0.15→0.10.
  - **Snapshot-diff vs pre-A1:** 36 score-endringer, **0 grade-flips** (godt under stop-criterion ≤5/asset-class). Topp Δ: SP500 SWING +0.147. Per asset-class indices/energy/metals 12 endringer hver, 0 flips; fx/crypto/softs/grains uendret.
  - **Live-driver-verifisering:** alle 5 nye drivere returnerer ikke-default-verdier (cboe_skew=0.6, vix9d=0.6, ovx_z=0.0, gvz=0.3, seismic_copper=0.75) — signaler aktive.
  - Pyright src/: 0 errors. Snapshot + engine-polarity-tester grønne. Diff-rapport: `docs/snapshot_diff_2026-05-02_followup_a1.md`.
  - 5 commits: `fb68e4d` SP500/Nasdaq risk, `0ab0c8a` CrudeOil/Brent macro, `16807b3` Gold macro, `5f97d45` Copper macro, `466c6a4` baseline+rapport.
- **Sub-fase 12.10 follow-up Spor A2 LUKKET 2026-05-02** (tag `v0.12.10-followup-a2`). Andre runde YAML-wirings — komplementerer A1 med lav-vekt-drivere (0.05-0.10) på samme 5-6 instrumenter. 6 driver-wirings:
  - CrudeOil + Brent macro: `eia_distillate_change@0.10` (bunke6 #22). dxy 0.25→0.20, vix 0.20→0.15.
  - Silver macro: `gvz_z@0.10`. real_yield 0.20→0.15, etf 0.20→0.15.
  - SP500 + Nasdaq risk: `vvix_z@0.05` (bunke4 #15, vol-of-vol). vol_regime 0.39→0.34.
  - Gold macro: `seismic_m6_global_24h@0.05` (bunke8 #28, global haven-trigger). real_yield 0.30→0.25.
  - **Snapshot-diff vs pre-A2:** 36 score-endringer, **0 grade-flips**, ingen score-Δ over 0.1. Per asset-class: indices/energy/metals 12/12/12 score-Δ med 0 flips; fx/crypto/softs/grains uendret.
  - **Akkumulert A1+A2: 14 wirings, 0 grade-flips totalt.** Pyright src/: 0 errors. Snapshot + engine-polarity + bot/systemd + mining_disruption-tester grønne.
  - Diff-rapport: `docs/snapshot_diff_2026-05-02_followup_a2.md`. 4 commits: `e442f6d` Crude/Brent eia_distillate, `0a51ac1` Silver gvz_z, `ec488e8` SP500/Nasdaq vvix_z, `4753a38` Gold seismic_m6.
- **Sub-fase 12.10 follow-up Spor A3 LUKKET 2026-05-02** (tag `v0.12.10-followup-a3`). Tredje runde — utvider til ikke-berørte asset-classes (Platinum + 4 FX + NaturalGas). 7 driver-wirings:
  - Platinum macro: `gvz_z@0.10` (PGM tracker gull-vol-syklus). dxy 0.35→0.30, mining_disruption 0.30→0.25.
  - EURUSD/GBPUSD/USDJPY/AUDUSD macro: `dollar_index_breadth@0.10` (bunke3 #13 — bredt USD-styrke-signal via 8 DEX-pairs). USDJPY: bull_when=high (invertert); de 3 andre: bull_when=low. real_yield 0.25→0.20, dxy 0.30→0.25 (eller AUDUSD: dxy 0.30→0.25, vix 0.25→0.20).
  - NaturalGas macro: `agsi_germany_pct@0.05` + `agsi_netherlands_pct@0.05` (bunke7 #23 thin wrappers — per-land EU-gass-fyllingsgrad). agsi_storage_pct 0.10→0.05, dxy 0.20→0.15.
...
- **Sub-fase 12.11 LUKKET 2026-05-02** — agri-engine horisont-bevisst + dropp SCALP for alle 7 agri + Sugar-ethanol-katalysator.
  ...
  - **Spor B (Energy-utvidelse for grains, NOT STARTED):** Plan for Corn + Soybean spesifikt (ikke alle agri). Begrunnelse: US-grains har ekte energi-koblinger som ikke fanges av positioning eller andre eksisterende familier:
    - **Corn** (~40% av US-mais til ethanol-blendingsmandat): legg til `momentum_z(instrument=CrudeOil)` @ 0.15-0.20 i cross-familien. Samme rasjonale som Sugar fikk i `4c5091a`, sterkere kobling for Corn. Trim eksisterende cross-vekter for sum=1.00.
    - **Soybean** (~10-15% biodiesel + soyaolje-→-biodiesel): legg til `momentum_z(instrument=CrudeOil)` @ 0.10 i cross. Mindre dominant enn Corn-ethanol.
    - **Naturgass-overlay (alle 3 grains)**: legg til `momentum_z(instrument=NaturalGas)` @ 0.05-0.10 i cross. Nat-gass driver urea/ammonia-pris (gjødsel) som påvirker bonde-margin og planting-beslutning. Mindre direkte enn olje, men reell.
    - Ikke relevant for: Cocoa/Coffee/Sugar/Cotton/Wheat (Sugar har allerede crude; Wheat har ikke ethanol-/biodiesel-mandat).
    - Estimert: 1 sub-session, 0 nye drivere (gjenbruk momentum_z), 2 YAML-endringer.
  - **Spor C (Supply-disruption for softs, NOT STARTED):** Plan for Cocoa/Coffee/Sugar/Cotton spesifikt. Begrunnelse: softs har akutt forsyningsrisiko (Vest-Afrika ~70% av cocoa-produksjon, Brasil-frost-risiko for coffee, shipping-flaskehalser for raw sugar/cotton). Vi har:
    - `iip_supply_unavailability` (10 628 rader Industrial Production-basert; designet for stress-deteksjon).
    - `mining_disruption` (per-region disruption-flagg; brukes for metaller men leser samme stress-mekanisme).
    - Vurder ny `supply_disruption`-familie (vekt 1, ikke 2 — supplerer ikke erstatter) eller utvid `cross`-familien per-instrument:
      - **Cocoa** primært: West-Africa political-risk + shipping → kandidat-driver `iip_supply_unavailability` med region-filter
      - **Coffee**: Brasil-frost-event-deteksjon (kald-front sub-zero ila vinter; krever ny eller utvidet `weather_stress`-mode `frost_risk`)
      - **Sugar/Cotton**: shipping-pressure er allerede fanget via cross.shipping_pressure (BDI-ETF). Mindre prioritet.
    - Estimert: 2-3 sub-sessioner, evt. 1 ny driver (`frost_risk_brazil`), 4 YAML-endringer.
    - **Lavere prioritet enn Spor B** — supply-disruption-drivere har sjeldne signal-events (event-baserte, ikke kontinuerlige), så empirisk verdi er vanskelig å kvantifisere før vi har data fra et faktisk event-vindu.
...
- **Sub-fase 12.10 follow-up Spor A4 LUKKET 2026-05-02** (tag `v0.12.10-followup-a4`). Agri-runde — leverer § 22.2 #30: erstatt `enso_regime` med `noaa_oni_index`. 1:1 driver-swap i 7 agri-instrumenter (Cocoa/Coffee/Sugar/Cotton/Corn/Soybean/Wheat enso-familien). Family-vekt + horisont uendret. Begge drivere leser identisk NOAA ONI-data — current ONI=-0.16 (neutral) gir bit-identisk score 0.5 i begge.
  ...
  - **Akkumulert A1-A4: 21 wirings + 7 replacements, 0 grade-flips totalt.** 22/22 instrumenter wired med 12.10-bunke-drivere. 6/9 bunker aktive (3, 4, 6, 7, 8, 9-defaults).
  - Diff-rapport: `docs/snapshot_diff_2026-05-02_followup_a4.md`. 1 commit: `4c67035`.
- **Sub-fase 12.10 follow-up Spor A5 LUKKET 2026-05-02** (tag `v0.12.10-followup-a5`). Bunke3 makro-utvidelser på indices + crypto. 6 wirings i 4 instrumenter:
  - SP500 + Nasdaq macro: `m2_yoy@0.05` + `t10y3m@0.05` (MAKRO-only). M2 YoY = likviditets-vekst; 10Y-3M = recession-leader. real_yield/dxy trimmet.
  - BTC + ETH macro: `m2_yoy@0.10` (MAKRO-only). Crypto er empirisk meget M2-likviditets-sensitiv (2020-21 bull-fase). real_yield 0.20→0.15, dxy 0.20→0.15.
  - **Snapshot-diff vs pre-A5:** 24 score-endringer, **0 grade-flips**, ingen score-Δ over 0.1. Per asset-class: indices/crypto 12/12 score-Δ med 0 flips; andre uendret.
  - **Akkumulert A1-A5: 27 wirings + 7 replacements, 0 grade-flips totalt.** 22/22 instrumenter wired; 6/9 bunker aktive.
  - Diff-rapport: `docs/snapshot_diff_2026-05-02_followup_a5.md`. 1 commit: `75b3cbf`.
- **Sub-fase 12.10 follow-up Spor A6 LUKKET 2026-05-02** (tag `v0.12.10-followup-a6`). Bunke6 EIA refinery-utilization. CrudeOil + Brent macro: `eia_refinery_utilization_z@0.10` (bunke6 #22). dxy 0.20→0.15, eia_stock 0.30→0.25. 12 score-Δ (energy), 0 flips.
- **Sub-fase 12.10 follow-up Spor A7 LUKKET 2026-05-02** (tag `v0.12.10-followup-a7`). Bunke6 EIA-utvidelse + bunke3 hy_oas_change. 7 wirings:
  - CrudeOil + Brent macro: `eia_imports_crude@0.05` + `eia_petroleum_supplied@0.05` (bunke6 #22). vix 0.15→0.10, eia_stock 0.25→0.20 for sum=1.00.
  - NaturalGas macro: `eia_propane_change@0.05` (bunke6 #22, invert=true). hdd_cdd 0.20→0.15.
  - SP500 + Nasdaq risk: `hy_oas_change@0.05` (bunke3 #8 MAKRO-only) — direkte HY-credit-stress, komplementerer credit_spread_change (Moody's AAA/BAA IG-credit). credit_spread_change 0.26→0.21.
  - **Snapshot-diff vs pre-A7:** 22 score-Δ, **0 grade-flips**, ingen Δ > 0.1. Per asset-class: energy 18 / indices 4 score-Δ med 0 flips.
  - **Akkumulert A1-A7: 36 wirings + 7 replacements, 0 grade-flips totalt.** 7/9 bunker har drivere i bruk (3, 4, 6, 7, 8, 9-defaults; bunke 5 er DEFERRED med ny YAML-utvidelser, bunke 1-2 var bug-fix-bunker).
  - 3 commits: `ab6a180` CrudeOil/Brent EIA imports+supplied, `364b817` NaturalGas eia_propane, `6fd2d7b` SP500/Nasdaq hy_oas_change.
- **Sub-fase 12.10 follow-up Spor A8 LUKKET 2026-05-02** (tag `v0.12.10-followup-a8`). Bunke3 labor + growth. 5 wirings:
  - SP500 + Nasdaq macro: `initial_claims_z@0.05` + `industrial_production_yoy@0.05` (begge MAKRO-only). vix 0.15→0.10, net_fed_liq 0.25→0.20.
  - Copper macro: `industrial_production_yoy@0.05` (Dr. Copper). dxy 0.30→0.25.
  - AUDUSD macro: `industrial_production_yoy@0.05` (commodity-FX). dxy 0.25→0.20.
  - **Snapshot-diff vs pre-A8:** 20 score-Δ, **0 grade-flips**, ingen Δ > 0.1. Per asset-class: indices 12 / metals 6 / fx 2 score-Δ med 0 flips.
  - **Akkumulert A1-A8: 41 wirings + 7 replacements, 0 grade-flips totalt.** 2 commits: `f64b2ca`, `9956426`.
- **Sub-fase 12.10 follow-up Spor A9 LUKKET 2026-05-02** (tag `v0.12.10-followup-a9`). Bunke3 sentiment + bunke4 bond-vol + bunke7 AGSI-utvidelse. 6 wirings:
  - SP500 + Nasdaq macro: `umich_sentiment_z@0.05` (consumer sentiment). real_yield 0.15→0.10.
  - SP500 + Nasdaq risk: `move_index_z@0.05` (treasury vol cross-check). vol_regime 0.34→0.29.
  - NaturalGas macro: `agsi_italy_pct@0.05` + `agsi_withdrawal_rate@0.05`. eia_stock 0.30→0.25, hdd_cdd 0.15→0.10.
  - **Snapshot-diff vs pre-A9:** 16 score-Δ, **0 grade-flips**, ingen Δ > 0.1. Per asset-class: indices 12 / energy 4 score-Δ med 0 flips.
  - Bug-fix på vei: første agsi-NaturalGas-commit (`69011dc`) overshjekket familie-sum=1.05 → fix `57cab30` trimmet eia_stock 0.30→0.25.
  - **Akkumulert A1-A9: 47 wirings + 7 replacements, 0 grade-flips totalt.** 3 commits: `a3bed44`, `69011dc`, `57cab30`.
- **Sub-fase 12.10 follow-up Spor A10 LUKKET 2026-05-02** (tag `v0.12.10-followup-a10`). Bunke3 fullføring + bunke6 fullføring + § 22.2 #11 ANFCI-replacement. 8 wirings (4 swap + 4 add):
  - SP500 + Nasdaq + BTC + ETH macro: `nfci_change → anfci_z` swap (per § 22.2 #11). 1:1 weight (0.20), samme bull_when=low. ANFCI er Adjusted NFCI (risiko-justert mot business-cycle-baseline).
  - SP500 + Nasdaq macro: `cfnai_3mma@0.05` + `jolts_openings_yoy@0.05` (siste bunke3 makro-utvidelser). real_yield 0.10→0.05, dxy 0.15→0.10.
  - CrudeOil + Brent macro: `eia_gasoline_demand@0.05` (siste bunke6 EIA-driver). eia_stock 0.20→0.15.
  - **Snapshot-diff vs pre-A10:** 46 score-Δ, **2 grade-flips** (Nasdaq SWING buy B→A 3.184→3.234; SP500 SWING buy B→A 3.185→3.235 — begge ved B/A-grensen 3.20, modest promotion fra ANFCI + cfnai-bidrag). Per asset-class: indices 12/2; metals 18/0 (drift, |Δ|<0.1); energy 12/0; crypto 4/0. **Stop-criterion ≤5/asset-class: OK.**
  - **Akkumulert A1-A10: 51 wirings + 11 replacements, 2 grade-flips totalt over 10 spor.** 4 commits: `536e821`, `6f047a5`, `be07174`.
  - **Bunke 6 100% wired** (alle 6 EIA-drivere), bunke 3 9/14 wired (resten har overlap eller liten verdi).
- **Sub-fase 12.10 follow-up Spor A11 LUKKET 2026-05-02** (tag `v0.12.10-followup-a11`). NaturalGas macro: `agsi_injection_rate@0.05`. hdd_cdd 0.10→0.05 for sum=1.00. **Bunke7 100% wired** (alle 5 AGSI-drivere). Snapshot-diff: 6 score-Δ (energy), 0 grade-flips.
- **Sub-fase 12.10 follow-up Spor A — TOTALT LUKKET 2026-05-02 (A1-A11).**
  - **52 wirings + 11 replacements totalt** — 22/22 instrumenter wired med 12.10-bunke-drivere.
  - **2 grade-flips totalt over 11 spor** (begge Nasdaq+SP500 SWING buy B→A i A10 ved B/A-grensen — modest promotion).
  - **Bunker fullt wired:** 6 (alle 6 EIA), 7 (alle 5 AGSI).
...
- **PLAN § 7.3 + § 7.4 systemd-drift fix 2026-05-02 (session 149):** Audit avdekket at PLAN-tabellene var frosset på session-110-snapshot. § 7.3 manglet 7 fetchere (`agsi`, `alsi`, `conab`, `unica`, `news_intel`, `crypto_sentiment`, `shipping`-rebrand fra `bdi`); § 7.4.2 hadde feil totaltall ("15 totalt" → reelt 21); § 7.4.3 manglet doblet-install-funn (calendar_ff + cot_ice er enabled både system + user med identisk OnCalendar = dobbel-fyring), og service-listen var ufullstendig (manglet `bedrock-bot`, `bedrock-monitor-alert`, `bedrock-signals-bot-morning`, `bedrock-notify@`, samt orphan-service `bedrock-signals-bot-intraday.service` uten timer som gir `Unit to trigger vanished`-warning). Alle tre tabeller oppdatert til faktisk virkelighet; aksjonsplanens preferanse flippet fra "system for prod" til "user for konsistens" siden 19/21 allerede er user-timers. Sanntids-status: 21 fetchere live + 4 system-services + 4 user-services + 1 orphan flagget for fix.
