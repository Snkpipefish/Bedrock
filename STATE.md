...
- **Sub-fase 12.5 ÅPEN 2026-04-25** — debt-rydding før observasjonsvinduet kan gi mening. Roadmap: Block A drivere (~3-4 sessioner), Block B agri-drivere (~4-5), Block C instrumenter (~3-5), Block D polish (~2-3). Totalt 12-17 sessioner.
  ...
  - **110-115:** én fetcher per session per § 7.5: cot_euronext (110), conab (111), unica (112), shipping (113 konsolidering med bdi), news_intel (114 UI-only), crypto_sentiment (115 UI-only).
...
- **Blocked:** nei.
- **Aktive systemd-timere:** 6 system-installerte (calendar_ff [session 105], cot_ice [session 106], signals-all, monitor, compare, server [service]) + 10 user-installerte (prices, cot_disaggregated/legacy, fundamentals, weather, crop_progress, bdi, eia_inventories [session 107], comex [session 108], **seismic [session 109, ny]**). Andre fetch-timer-units (enso, wasde) er generert i `/home/pc/bedrock/systemd/` men IKKE aktivert.
...
- **Drivere:** 27 registrert (26 + `mining_disruption` ny session 109).
- **Bedrock-fetchere:** 14 totalt (prices, cot_disaggregated, cot_legacy, fundamentals, weather, enso, wasde, crop_progress, bdi, calendar_ff [session 105], cot_ice [session 106], eia_inventories [session 107], comex [session 108], **seismic [session 109]**). 6 gjenstår å port per § 7.5 (sessions 110-115).
...
- **Sub-fase 12.5+ scope:** 11 ikke-portede cot-explorer-fetchere (PLAN § 7.5) + ADR-007 strategi + ADR-008 mapping. Sessions 105-109 lukket (5/11). Sessions 110-115 (6/11) + 116-117 (Phase D) gjenstår.
...
- **Next task:** **Session 110** — `fetch_euronext_cot.py` → `bedrock/fetch/cot_euronext.py` med `cot_euronext_mm_pct`-driver for Wheat/Corn (EU-overlay). MiFID II-kategorier (Investment Funds ≈ MM). PLAN § 7.5 rad 6.
