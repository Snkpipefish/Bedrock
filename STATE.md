...
- **Sub-fase 12.5 ÅPEN 2026-04-25** — debt-rydding før observasjonsvinduet kan gi mening. Roadmap: Block A drivere (~3-4 sessioner), Block B agri-drivere (~4-5), Block C instrumenter (~3-5), Block D polish (~2-3). Totalt 12-17 sessioner.
  ...
  - **111-115:** én fetcher per session per § 7.5: conab (111), unica (112), shipping (113 konsolidering med bdi), news_intel (114 UI-only), crypto_sentiment (115 UI-only).
...
- **Blocked:** nei.
- **Aktive systemd-timere:** 6 system-installerte (calendar_ff [session 105], cot_ice [session 106], signals-all, monitor, compare, server [service]) + 11 user-installerte (prices, cot_disaggregated/legacy, fundamentals, weather, crop_progress, bdi, eia_inventories [session 107], comex [session 108], seismic [session 109], **cot_euronext [session 110, ny]**). Andre fetch-timer-units (enso, wasde) er generert i `/home/pc/bedrock/systemd/` men IKKE aktivert.
...
- **Drivere:** 28 registrert (27 + `cot_euronext_mm_pct` ny session 110).
- **Bedrock-fetchere:** 15 totalt (prices, cot_disaggregated, cot_legacy, fundamentals, weather, enso, wasde, crop_progress, bdi, calendar_ff [session 105], cot_ice [session 106], eia_inventories [session 107], comex [session 108], seismic [session 109], **cot_euronext [session 110]**). 5 gjenstår å port per § 7.5 (sessions 111-115).
...
- **Sub-fase 12.5+ scope:** 11 ikke-portede cot-explorer-fetchere (PLAN § 7.5) + ADR-007 strategi + ADR-008 mapping. Sessions 105-110 lukket (6/11). Sessions 111-115 (5/11) + 116-117 (Phase D) gjenstår.
...
- **Comex_inventory DB:** 3 rader backfilt fra metalcharts.org v2-API ved session 108 (Gold/Silver/Copper @ 2026-04-24). Refresh via user-systemd-timer Mon-Fri 22:00 Oslo med smart-skip basert på `_previous_business_day()`. Default-fetcher henter kun siste rad — historikk bygger seg over tid via daglig timer-cron (cot-explorer-presedens; metalcharts har ikke historikk-endepunkt).
- **Seismic_events DB:** 100 events backfilt fra USGS @ 2026-04-27 ved session 109 (M≥4.5 siste 7d, 13 i mining-regions). Refresh via user-systemd-timer daglig 04:00 Oslo. Idempotent på event_id — INSERT OR REPLACE håndterer USGS-revisjoner naturlig.
- **Cot_euronext DB:** 174 rader backfilt fra Euronext @ 2026-04-27 ved session 110 (58 onsdager × 3 produkter — milling wheat, corn, canola — 2025-03-05 til 2026-04-22). Refresh via user-systemd-timer Ons 18:00 Oslo med smart-skip basert på `_previous_wednesday()`.
- **Next task:** **Session 111** — `fetch_conab.py` → `bedrock/fetch/conab.py` med `conab_yoy`-driver for Corn/Soybean/Coffee. Brazil crop estimates via PDF (poppler-utils + pypdf-fallback per ADR-007 § 6). PLAN § 7.5 rad 7.
