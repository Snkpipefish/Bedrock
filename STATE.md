...
- **Sub-fase 12.5 ÅPEN 2026-04-25** — debt-rydding før observasjonsvinduet kan gi mening. Roadmap: Block A drivere (~3-4 sessioner), Block B agri-drivere (~4-5), Block C instrumenter (~3-5), Block D polish (~2-3). Totalt 12-17 sessioner.
  ...
  - **117:** Phase D fortsettelse — ADR-009 cutover-readiness + tag `v0.12.5-fetch-port-complete`.
...
- **Next task:** **Session 117** — Phase D fortsettelse: ADR-009 cutover-readiness-audit (gjennomgang av Phase A-C-driverbidrag empirisk; vurdering av om news_intel/crypto_sentiment-drivere skal aktiveres med vekt 0.05-0.1 nå eller vente på mer data; vurdering av asymmetrisk publish-floor-utvidelse til Sugar BUY (ny evidence: 0% hit-rate ved 81.8/87.5% pub-rate); cutover-checklist; vurdering av Nivå 3 git-modus). Avsluttes med tag `v0.12.5-fetch-port-complete`.
