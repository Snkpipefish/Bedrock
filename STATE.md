...
- **Next task:** **Sub-fase 12.6 oppdelt i 2 sessioner per audit-runde 5 (`c87e278`).** Strategi 2 (fix-first + re-harvest) anbefalt etter harvest-completion — Strategi 3 (filter analyzer) deprecated.
  - **Session 137 = event_distance pre-rebalanserings-fix + re-harvest** (Steg 1+2+4 av 6-stegs-plan):
    1. **Type A** (Engine `_now`-propagering): utvid `engine.py:Engine.score(...)` med `now: datetime | None = None`-param; `_score_families` legger `_now=now.isoformat() if now else None` i `params_with_dir`; `signals.py:_compute_scores(...)` propagerer `now=run_ts` til `eng.score(...)`. Behold `risk.py:201-205` wallclock-fallback. 2 nye tester i `tests/unit/test_engine_now_propagation.py`.
    2. **Type B** (Forex Factory publikasjons-lag): legg til `--publication-lag-days INT` (default 7) i `scripts/ingest_manual_data.py:91-97`. Endre `filtered["fetched_at"] = filtered["event_ts"]` til `... = event_ts - pd.Timedelta(days=publication_lag_days)`. Re-import forex-CSV med `--publication-lag-days 7`. Semantikk-valg: (c) "publikasjons-tidspunkt" — Forex Factory publiserer kalenderen ~7 dager før event = korrekt look-ahead-fri backtest-semantikk. (a) import-tidspunkt og (b) event_ts forkastet (se audit-doc Sjekk 9.5 Steg 2 lines 234-247).
    3. ~~**Type C**~~: HOPP OVER. `min_hours=4` er intensjonell live-design, ikke uavhengig bug. Re-evaluer kun hvis IC=0 etter A+B (presisert audit-runde 4).
    4. **Backfill**: `DELETE FROM driver_observations WHERE driver_name='event_distance'` (~3153 rader). Re-kjør `harvest_driver_observations.py` for alle 22 instrumenter — kun event_distance får write-pressure pga INSERT OR IGNORE. ETA ~1-2t. Verifisér: ≥5 distinct values, avg ≠ 1.0.
    Stop-criterion: A+B fixet, tester grønne, event_distance-rader backfilled, snapshot-diff dokumenterer score-endringer for berørte instrumenter (Brent/CrudeOil/Cocoa/etc.). Estimert 4-6t.
  - **Session 138 = analyzer-execution + YAML-rebalansering + dead-driver-cleanup** (Steg 5+6+6.5):
    5. **Analyzer-runde**: `analyze_driver_performance.py` (IC per driver per instrument×horizon×direction, krev ≥50 obs) + `analyze_cross_correlations.py` (cross-corr matrix per familie). Output: `docs/12_6_ic_table.md` + `docs/12_6_cross_correlation.md`.
    ...
    Stop-criterion: rebalansering-YAMLs på main, snapshot-baseline ny anker, dead drivers ute. Estimert 6-8t.
