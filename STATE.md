...
- **Next task:** **Session 137 = pre-rebalanserings-fix av event_distance trippel-bug + analyzer-execution + YAML-rebalansering** når Codespace-harvest er ferdig (~24t ETA fra 2026-04-30 ~20:04). Notify-send-popup på laptop signaliserer completion. Henteflyt: `gh codespace cp` DB tilbake fra VM, fix event_distance (Type A+B per audit Sjekk 9.5), backfill driver_observations.event_distance-radene, kjør `analyze_driver_performance.py` + `analyze_cross_correlations.py`, rebalansér YAML-vekter per `docs/12_6_analyzer_plan.md`. Stop codespace etter DB-hent for å spare core-hours (24h × 4-core = 96 core-hours, innenfor 120 free/mnd). Open tech-gjeld: **event_distance trippel-bug (PRE-REBALANSERINGS-BLOCKER, eskalert audit-runde 3 — se under)**, FRED-fetcher hard-fail-policy, PPLT SEC EDGAR Plan-S, NOPA WASDE-utvidelse, CONAB Café-PDF-historikk (KRITISK 3), `src/bedrock/fetch/fas_esr.py` L134 stale docstring.
