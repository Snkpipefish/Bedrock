...
- **Sub-fase 12.5 ÅPEN 2026-04-25** — debt-rydding før observasjonsvinduet kan gi mening. Roadmap: Block A drivere (~3-4 sessioner), Block B agri-drivere (~4-5), Block C instrumenter (~3-5), Block D polish (~2-3). Totalt 12-17 sessioner.
  ...
  - **112-115:** én fetcher per session per § 7.5: unica (112), shipping (113 konsolidering med bdi), news_intel (114 UI-only), crypto_sentiment (115 UI-only).
...
- **Blocked:** nei.
- **Aktive systemd-timere:** 6 system-installerte (calendar_ff [session 105], cot_ice [session 106], signals-all, monitor, compare, server [service]) + 14 user-installerte (prices, cot_disaggregated/legacy, fundamentals, weather, enso, wasde, crop_progress, bdi, eia_inventories [session 107], comex [session 108], seismic [session 109], cot_euronext [session 110], **conab [session 111, ny]**). Alle 16 fetch-relaterte timere er nå aktivt installert.
...
- **Drivere:** 29 registrert (28 + `conab_yoy` ny session 111).
- **Bedrock-fetchere:** 16 totalt (prices, cot_disaggregated, cot_legacy, fundamentals, weather, enso, wasde, crop_progress, bdi, calendar_ff [session 105], cot_ice [session 106], eia_inventories [session 107], comex [session 108], seismic [session 109], cot_euronext [session 110], **conab [session 111]**). 4 gjenstår å port per § 7.5 (sessions 112-115).
...
- **Sub-fase 12.5+ scope:** 11 ikke-portede cot-explorer-fetchere (PLAN § 7.5) + ADR-007 strategi + ADR-008 mapping. Sessions 105-111 lukket (7/11). Sessions 112-115 (4/11) + 116-117 (Phase D) gjenstår.
...
- **Next task:** **Session 112** — `fetch_unica.py` → `bedrock/fetch/unica.py` med `unica_change`-driver for Sugar (PDF via poppler-utils per ADR-007 § 6). UNICA = Brazilian sugar/ethanol industry association. PLAN § 7.5 rad 8.
