...
- **Sub-fase 12.5 ÅPEN 2026-04-25** — debt-rydding før observasjonsvinduet kan gi mening. Roadmap: Block A drivere (~3-4 sessioner), Block B agri-drivere (~4-5), Block C instrumenter (~3-5), Block D polish (~2-3). Totalt 12-17 sessioner.
  ...
  - **118+:** Sub-fase 12.6 "data-driven rebalansering" — vente på harvest, sesong-bucketing-analyzer, setup-walker, YAML-rebalansering basert på empiri.
...
- **Next task:** **Session 118** — etter harvest ferdig: kjøre `analyze_driver_performance.py` + `analyze_cross_correlations.py`, identifisere svake/sterke drivere, foreslå første runde YAML-vekt-rebalansering. Senere sessions: legg til sesong-bucketing + lead-lag-IC i analyzer, bygg setup-walker for ekte P&L-distribusjon. Iterer til konvergens.
